...
const Orcamentos = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Generate an ID and created_at locally so we can show/download the quote PDF
      const generatedId = crypto.randomUUID();
      const createdAt = new Date().toISOString();

      // Build payload; include created_by only if authenticated
      ...
      const dbPayload: any = creatorId
        ? { ...basePayload, created_by: creatorId }
        : basePayload;

      // Insert without .select() to avoid RETURNING/SELECT RLS issues for anon users
      // Avoid return=representation which requires SELECT permission under RLS
      const { error } = await supabase
        .from("quotes")
        .insert([dbPayload], { returning: "minimal" });

      if (error) throw error;

      // Build a client-side quote object compatible with generateQuotePDF
      const clientQuote = {
        id: generatedId,
        customer_name: dbPayload.customer_name,
        customer_email: dbPayload.customer_email,
        customer_phone: dbPayload.customer_phone,
        items: quoteItems.map((it: any) => ({
          product_name: it.product_name,
          product_type: it.product_type,
          quantity: it.quantity,
        })),
        total_value: dbPayload.total_value,
        status: dbPayload.status,
        notes: dbPayload.notes,
        created_at: createdAt,
      };

      setLastCreatedQuote(clientQuote);
